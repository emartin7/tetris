    ...
    implements Brain
{
    ...
    public
        void
        tryAllColumns(Board board, Piece piece, int heightLimit, Move move)
    {
        int column = 0;
    ...
    public double rateBoard(Board board)
    {
        ...
        totalScore = blockedHolesOnBoard * 10000 + openSidedHolesOnBoard * 7500
            ...
            + ((heightDisparity * heightDisparity / 2) * 200)
            + weightedHeightScore * 200;
        return totalScore;
        ...
        for (int column = 0; column < board.getWidth(); column++)
        {
            ...
            for (int row = 0; row < height; row++)
            {
                if (!board.hasBlockAt(column, row))
                {
                    ...
                    {
                        if ((board.getColumnHeight(column - 1) <= row)
                            || (board.getColumnHeight(column + 1) <= row))
                        {
                            numOfOpenSidedHoles++;
                        }
                        else
    ...
    private int getDisparity(Board board)
    {
        ...
        {
            localDisparity += Math.abs(
                board.getColumnHeight(column)
                    - board.getColumnHeight(column - 1));
        }
    ...
    private int getAggregateHeightOfBoard(Board board)
    {
        ...
        for (int column = 0; column < board.getWidth(); column++)
        {
            weightedAggregateHeight +=
                (board.getColumnHeight(column) * board.getColumnHeight(column));
        }
